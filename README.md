### This web app was created using Next.js and Hugging Face API,

### The time it takes for the LLM to generate essays will vary depending on the response length and compute time, if it takes a bit longer simply wait for the API call to be completed.

### Visit the link below to see the app in action:
 https://ai-college-essay-assistant.vercel.app/

## Known Bugs:
Generating may throw a JSON related error, this seems to stem from Vercel's deployment or how the environmental variables are being loaded, this issues is generally fixed by either clicking the generate button again or reloading the page
